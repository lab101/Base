...
void NotificationManager::addMessage(std::string message, Color color){
   ...
   // try{
    ...
    notifications.push_back(p);
  	...
  	p->setDataForMainThread(getTimeString() + "    " + message, color);
		...
		if (!n->markedForRemoval){
			if (!n->mTexture)
				try{
					...
					simple.setColor(n->mColor);
					simple.addLine(n->mText);
					n->setTexture(gl::Texture2d::create(simple.render(true, false)));

					const float textureHeight = n->getTextureHeight();
					for (std::shared_ptr<Notification> n2 : notifications)
					{
						n2->move(textureHeight);
					}

					n->fadeIn(vec2(400, 10));
			}
			catch (...){
